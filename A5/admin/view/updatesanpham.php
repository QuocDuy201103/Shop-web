    ...
    <div class="home-content">
        ...
        <div class="row ">
            ...
            <div class="row frmcontent">
                ...
                <!-- modal nhap them-->
                <form action="index.php?act=sanpham_update" method="post" class="modal js-modal" enctype="multipart/form-data">
                    <div class="modal-container js-modal-container">
                        <div class="modal-close js-modal-close">
                            <i class="bi bi-x-circle"></i>
                        </div>
                        <header class="modal-heading">
                            <i class="modal-heading-icon fa-solid fa-suitcase"></i> Cập nhật sản phẩm
                        </header>
                        <div class="modal-body">
                            ...
                            <select name="iddanhmuc" id="" class="modal-input">
                                ...
                                <?php
                                $iddanhmuc_cur = $spct[0]['iddanhmuc'];
                                if (isset($dsdm)) {
                                    foreach ($dsdm as $dm) {
                                        if ($dm['id'] == $iddanhmuc_cur)
                            ...
                            <label for="modal-ticket-name" class="modal-label">
                                ...
                                Tên sản Phẩm</label>
                            <input type="modal-email" id="modal-ticket-name" class="modal-input" name="tensanpham" value="<?= $spct[0]['tensanpham'] ?>">

                            <label for="modal-ticket-picture" class="modal-label">
                                <i class="bi bi-image"></i>
                                Ảnh sản phẩm</label>
                            <input class="modal-input" type="file" name="img" id="upFile">
                            <img src="<?= $spct[0]['img'] ?>" width="50" alt="">

                            <label for="modal-ticket-price" class="modal-label">
                                <i class="bi bi-currency-dollar"></i>
                                Giá sản Phẩm</label>
                            <input type="modal-email" id="modal-ticket-price" class="modal-input" name="gia" value="<?= $spct[0]['gia'] ?>">

                            <label for="modal-ticket-amount" class="modal-label">
                                <i class="bi bi-geo-alt-fill"></i>
                                Xuất xứ</label>
                            <input type="text" id="modal-ticket-origin" class="modal-input" name="xuatxu" value="<?= $spct[0]['xuatxu'] ?>">

                            <label for="modal-ticket-amount" class="modal-label">
                                <i class="bi bi-droplet-fill"></i>
                                Dung tích</label>
                            <input type="text" id="modal-ticket-origin" class="modal-input" name="dungtich" value="<?= $spct[0]['dungtich'] ?>">

                            <label for="modal-ticket-amount" class="modal-label">
                                <i class="bi bi-chat-left-text"></i>
                                Mô tả</label>
                            <input type="text" id="modal-ticket-desc" class="modal-input" name="detail" value="<?= $spct[0]['detail'] ?>">

                            <input type="hidden" name="id" id="" value="<?= $spct[0]['id'] ?>">

                            <input id="buy-ticket" type="submit" name="updatesanpham" value="Cập nhật sản phẩm">
                        </div>
                    </div>
                </form>
                <!-- end modal nhap them -->
                                ...
                                echo '
                                        <tr>
                                            ...
                                            <div style="margin-bottom:10px;">
                                                ...
                                                <a class="hide-pro" href="index.php?act=ansanpham&id=' . $item['id'] . '">' . (($item['status'] == 1) ? 'Hiển sản phẩm' : 'Ẩn sản phẩm') . '</a>
                                            </td>
                                            </td>
                                        </tr>';
                                $i++;
